...
| --------------------- | ------------- | ------------------------------------------------------------------------------------------------ | ------ |
| **Redis Caching**     | **Done**       | Two-layer cache-aside (BFF L1 20ms/10ms, Invoice L2 30ms/15ms) with Redis + ioredis INCR invalidation. | Medium |
| **Rate Limiting**     | **Done**       | Custom `@libs/rate-limit` with token bucket Lua script, per-endpoint `@RateLimit()`, trust proxy, 429 RFC 7807 responses. | Small  |
| **Circuit Breaker**   | **Done**       | `@libs/circuit-breaker` with CLOSED/OPEN/HALF_OPEN state machine, `@Optional()` injection in BaseTcpClient, cache fallback for reads, X-Circuit-State headers. | Medium |
| **Cursor Pagination** | **Done**       | Dual-mode offset+cursor pagination with `MutuallyExclusivePageCursor` validator, base64url cursor encoding. | Medium |
...
- [x] Redis caching for `findOne` and hot `findAll` (`@libs/cache`, cache-aside with ioredis INCR invalidation)
- [x] Custom rate limiting on BFF endpoints (`@libs/rate-limit`, token bucket, per-endpoint `@RateLimit()`)
- [x] Circuit breaker for downstream RPC calls (`@libs/circuit-breaker`, state machine, cache fallback)
- [x] Cursor-based pagination (dual-mode offset+cursor, `MutuallyExclusivePageCursor` validator)
...
_Document version: 2026-05-26_
_Last reviewed after commit: circuit breaker implementation complete_
